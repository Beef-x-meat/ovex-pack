...
const defaultSizesByCategory = {
  Becher: ['8oz', '12oz', '16oz'],
  Boxen: ['500ml', '750ml', '1000ml'],
  Taschen: ['S', 'M', 'L'],
  Deckel: ['80mm', '90mm'],
  Wraps: ['30x30cm', '35x35cm']
};
    ...
    <div className="max-w-7xl mx-auto px-6 lg:px-8 py-12" data-testid="page-product-detail">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground mb-8 flex-wrap">
        ...
        {categorySlug && (
          <>
            <span>/</span>
            <Link href={`/produkte?cat=${categorySlug}`} className="hover:text-foreground transition-colors">
              {product.category}
            </Link>
          </>
        )}
